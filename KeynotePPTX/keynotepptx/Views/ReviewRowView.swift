...
#Preview {
    ...
    // Create mock PPTXMediaItem
    let pptxItem = PPTXMediaItem(
        filename: "image1.jpg",
        absolutePath: URL(fileURLWithPath: "/tmp/image1.jpg"),
        slideNumbers: [1, 2],
        isMasterOnly: false
    )

    // Create mock ImageFingerprint
    ...
    // Create mock CandidateMatch
    let candidate = CandidateMatch(
        keynoteFilename: "image.svg",
        keynotePath: URL(fileURLWithPath: "/tmp/image.svg"),
        fileExtension: "svg",
        fileSizeBytes: 54321,
        thumbnailData: nil,
        aHashDistance: 5,
        pHashDistance: 10,
        colorMomentDistance: 0.2,
        replacementKind: .svg
    )

    // Create mock MappingRow
    let row = MappingRow(
        pptxItem: pptxItem,
        pptxFingerprint: fingerprint,
        topCandidates: [candidate],
        quality: .strong,
        isXmlExact: false,
        selectedChoice: .keynoteFile(filename: candidate.keynoteFilename)
    )

    appState.mappingRows = [row]

    return ReviewRowView(rowIndex: 0)
